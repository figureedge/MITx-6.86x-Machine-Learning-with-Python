...
def project_onto_PC(X, pcs, n_components):
    ...
    # X (n, d) 
    # V (d, k) k eigen-vectors, d >= k
    # pcs (d, d) full components of eigen-vector 
    # P (n, n)

    P = np.matmul(center_data(X), pcs)

    # keep the n dim and select only n dim from k features 
    # only hold for d > n
    return P[:,0:n_components]
